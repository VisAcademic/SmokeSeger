...
from paddleseg.utils import on_ai_studio
...
def evaluate(model,
    ...
    if record_detail:
        # Record val results
        val_info = f'[EVAL] #Images: {len(eval_dataset)} ' \
                   f'mIoU: {miou:.4f} ' \
                   f'Acc: {acc:.4f} ' \
                   f'Kappa: {kappa:.4f}\n' \
                   f'[EVAL] Class IoU: {np.round(class_iou, 4)} \n' \
                   f'[EVAL] Class Acc: {np.round(class_acc, 4)} \n'
        if is_few_shot:
            with open(f'{save_info}_few_shot_val.log', 'a+') as f:
                f.write(val_info)
        else:
            save_path = f'{save_info}_val.log'
            if on_ai_studio():
                save_path = f'work/{save_info}_val.log'
