...
class Config(object):
    ...
    def lr_scheduler(self) -> paddle.optimizer.lr.LRScheduler:
        ...
        # WarmUp LR with inner LR scheduler
        learning_rate = params.get('learning_rate')
        if isinstance(learning_rate, dict):
            inner_lr_params = params.pop('learning_rate')
            inner_lr_type = inner_lr_params.pop('type')
            if inner_lr_type == 'PolynomialDecay':
                inner_lr_params.setdefault('decay_steps', self.iters)
                inner_lr_params.setdefault('end_lr', 0)
                inner_lr_params.setdefault('power', 0.9)
            learning_rate = \
                getattr(paddle.optimizer.lr, inner_lr_type)(**inner_lr_params)
            return \
                getattr(paddle.optimizer.lr, lr_type)(learning_rate=learning_rate,
                                                      **params)
    ...
    @property
    def few_shot_val_dataset_config(self) -> Dict:
        return self.dic.get('few_show_val_dataset', {}).copy()
    ...
    @property
    def few_shot_val_dataset(self) -> paddle.io.Dataset:
        _few_shot_val_dataset = self.few_shot_val_dataset_config
        if not _few_shot_val_dataset:
            return None
        return self._load_object(_few_shot_val_dataset)
